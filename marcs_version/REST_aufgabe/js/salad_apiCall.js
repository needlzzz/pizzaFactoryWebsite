...
const xhrSalad = new XMLHttpRequest();
        ...
        if (xhrSalad.readyState === 4) {
            if (xhrSalad.status === 200) {
                ...
                for (let i = 0; i < jsonSalad.length; i++) {
                    ...
                    boxSalad += '<h2 class="subtitle">'+jsonSalad[i]['name']+'</h2>';
                    ...
                    boxSalad += '</select>'
                    boxSalad += '<button onclick="orderBtn()"  class="feature-icon"><i class="fa fa-cart-arrow-down" aria-hidden="true"></i></button>';
    ...
    function orderBtn() {
        xhrSalad.open("POST", "https://tonyspizzafactory.herokuapp.com/api/orders", true);
        xhrSalad.setRequestHeader("Authorization", "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.MQ.bYceSpllpyYQixgNzDt7dpCkEojdv3NKD-85XLXfdI4", true);
